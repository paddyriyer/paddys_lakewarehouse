...
// ═══ THEME ═══
const C = {bg:"#0B1120",panel:"#111827",card:"#1A2332",card2:"#1F2D40",border:"#2A3A50",text:"#E2E8F0",muted:"#8B9DB7",dim:"#5A6B80",
...
const ttStyle = {background:C.panel,border:`1px solid ${C.border}`,borderRadius:8,fontSize:11,color:C.text};
...
const KPI = ({label,value,sub,color=C.accent,trend,icon=""}) => (
  <div style={{background:C.card,borderRadius:10,padding:"14px 16px",border:`1px solid ${C.border}`,flex:1,minWidth:130,position:"relative",overflow:"hidden"}}>
...
const Card = ({title,children,span=1,h}) => (
  <div style={{background:C.card,borderRadius:10,border:`1px solid ${C.border}`,padding:"14px 16px",gridColumn:`span ${span}`,display:"flex",flexDirection:"column",minHeight:h||"auto"}}>
...
export default function Dashboard() {
  ...
  return (
    <div style={{background:C.bg,minHeight:"100vh",fontFamily:"'DM Sans',-apple-system,sans-serif",color:C.text}}>
      ...
      {/* Header */}
      <div style={{background:"linear-gradient(90deg,#111827,#1A2332)",borderBottom:`1px solid ${C.border}`,padding:"10px 20px",display:"flex",alignItems:"center",justifyContent:"space-between"}}>
        <div style={{display:"flex",alignItems:"center",gap:12}}>
          <div style={{width:32,height:32,borderRadius:8,background:"linear-gradient(135deg,#FF9900,#8B5CF6)",display:"flex",alignItems:"center",justifyContent:"center",fontWeight:800,fontSize:14,color:"#fff"}}>S</div>
          <div>
      ...
      {/* Tabs */}
      <div style={{display:"flex",gap:0,padding:"0 12px",background:C.panel,borderBottom:`1px solid ${C.border}`,overflowX:"auto"}}>
        {tabs.map((t,i)=>(
  ...
  return (<div style={{display:"flex",flexDirection:"column",gap:14}}>
    ...
    <div style={{display:"grid",gridTemplateColumns:"2fr 1fr",gap:12}}>
      <Card title="📊 Real-Time Traffic — Today (Hourly)">
        <ResponsiveContainer width="100%" height={240}>
          <ComposedChart data={rtData}>
            ...
            <XAxis dataKey="h" tick={{fill:C.muted,fontSize:10}}/>
      ...
      <Card title="🏢 Revenue by Segment ($M)">
        <ResponsiveContainer width="100%" height={240}>
          <PieChart><Pie data={segments} cx="50%" cy="50%" innerRadius={50} outerRadius={85} dataKey="rev" label={({name,percent})=>`${name} ${(percent*100).toFixed(0)}%`} style={{fontSize:9}}>
    ...
    <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:12}}>
      <Card title="🔗 Pipeline Funnel">
        <ResponsiveContainer width="100%" height={200}>
          <BarChart data={pipeline} layout="vertical" margin={{left:15}}>
            ...
            <YAxis type="category" dataKey="stage" tick={{fill:C.muted,fontSize:9}} width={65}/>
            <Tooltip contentStyle={ttStyle}/>
      ...
      <Card title="🛡️ Fraud by Severity">
        <ResponsiveContainer width="100%" height={200}>
          <PieChart><Pie data={fraudSev} cx="50%" cy="50%" innerRadius={40} outerRadius={70} dataKey="value" label={({name,value})=>`${name}: ${value}`} style={{fontSize:9}}>
  ...
  return (<div style={{display:"flex",flexDirection:"column",gap:14}}>
    ...
    <Card title="📊 Monthly Revenue & Profit ($M)">
      <ResponsiveContainer width="100%" height={280}>
        <ComposedChart data={monthly}><CartesianGrid strokeDasharray="3 3" stroke={C.border}/><XAxis dataKey="m" tick={{fill:C.muted,fontSize:8}} interval={2}/>
          <YAxis tick={{fill:C.muted,fontSize:9}}/><Tooltip contentStyle={ttStyle}/><Legend wrapperStyle={{fontSize:10}}/>
    ...
    <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:12}}>
      <Card title="📦 Revenue by Category ($M)">
        <ResponsiveContainer width="100%" height={220}>
          <BarChart data={categories} layout="vertical" margin={{left:15}}><XAxis type="number" tick={{fill:C.muted,fontSize:9}}/><YAxis type="category" dataKey="name" tick={{fill:C.muted,fontSize:9}} width={65}/><Tooltip contentStyle={ttStyle}/>
      ...
      <Card title="🌍 Revenue by Country ($M)">
        <ResponsiveContainer width="100%" height={220}>
          <BarChart data={countries}><CartesianGrid strokeDasharray="3 3" stroke={C.border}/><XAxis dataKey="c" tick={{fill:C.muted,fontSize:9}}/><YAxis tick={{fill:C.muted,fontSize:9}}/><Tooltip contentStyle={ttStyle}/>
...
function Cust360() {
  ...
  return (<div style={{display:"flex",flexDirection:"column",gap:14}}>
    ...
    <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:12}}>
      <Card title="👥 Customer Status"><ResponsiveContainer width="100%" height={200}><PieChart><Pie data={statusData} cx="50%" cy="50%" innerRadius={45} outerRadius={75} dataKey="value" label={({name,percent})=>`${name} ${(percent*100).toFixed(0)}%`} style={{fontSize:9}}>{statusData.map((e,i)=><Cell key={i} fill={e.color}/>)}</Pie><Tooltip contentStyle={ttStyle}/></PieChart></ResponsiveContainer></Card>
      <Card title="💬 Sentiment"><ResponsiveContainer width="100%" height={200}><PieChart><Pie data={sentimentData} cx="50%" cy="50%" innerRadius={45} outerRadius={75} dataKey="value" label={({name,percent})=>`${name} ${(percent*100).toFixed(0)}%`} style={{fontSize:9}}>{sentimentData.map((e,i)=><Cell key={i} fill={e.color}/>)}</Pie><Tooltip contentStyle={ttStyle}/></PieChart></ResponsiveContainer></Card>
      <Card title="🏢 Revenue by Segment"><ResponsiveContainer width="100%" height={200}><RadarChart data={segments} cx="50%" cy="50%" outerRadius={60}><PolarGrid stroke={C.border}/><PolarAngleAxis dataKey="name" tick={{fill:C.muted,fontSize:9}}/><Radar dataKey="rev" stroke={C.accent} fill={C.accent} fillOpacity={.3}/><Radar dataKey="cust" stroke={C.purple} fill={C.purple} fillOpacity={.2}/><Tooltip contentStyle={ttStyle}/></RadarChart></ResponsiveContainer></Card>
    </div>
  </div>);
}
    ...
    <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:12}}>
      <Card title="🔄 Lifecycle Stage Distribution">
        <ResponsiveContainer width="100%" height={260}>
          <BarChart data={lcStages}><CartesianGrid strokeDasharray="3 3" stroke={C.border}/><XAxis dataKey="name" tick={{fill:C.muted,fontSize:10}}/><YAxis tick={{fill:C.muted,fontSize:10}}/><Tooltip contentStyle={ttStyle}/>
      ...
      <Card title="🎯 Churn Risk Tiers">
        <ResponsiveContainer width="100%" height={260}>
          <PieChart><Pie data={churnTiers} cx="50%" cy="50%" innerRadius={55} outerRadius={95} dataKey="value" label={({name,value})=>`${name}: ${value}`} style={{fontSize:11}}>{churnTiers.map((e,i)=><Cell key={i} fill={e.color}/>)}</Pie><Tooltip contentStyle={ttStyle}/></PieChart>
        </ResponsiveContainer>
      </Card>
    </div>
  ...
  return (<div style={{display:"flex",flexDirection:"column",gap:14}}>
    ...
    <div style={{display:"grid",gridTemplateColumns:"2fr 1fr",gap:12}}>
      <Card title="🔗 Pipeline Funnel — Deals by Stage">
        <ResponsiveContainer width="100%" height={260}>
          <ComposedChart data={pipeline}><CartesianGrid strokeDasharray="3 3" stroke={C.border}/><XAxis dataKey="stage" tick={{fill:C.muted,fontSize:9}}/><YAxis tick={{fill:C.muted,fontSize:9}}/><YAxis yAxisId="r" orientation="right" tick={{fill:C.muted,fontSize:9}}/><Tooltip contentStyle={ttStyle}/><Legend wrapperStyle={{fontSize:10}}/>
      ...
      <Card title="❌ Loss Reasons">
        <ResponsiveContainer width="100%" height={260}>
          <BarChart data={lossReasons} layout="vertical" margin={{left:15}}><XAxis type="number" tick={{fill:C.muted,fontSize:9}}/><YAxis type="category" dataKey="reason" tick={{fill:C.muted,fontSize:10}} width={80}/><Tooltip contentStyle={ttStyle}/>
    ...
    <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:12}}>
      ...
      <Card title="📡 Lead Source Performance">
        <ResponsiveContainer width="100%" height={230}>
          <BarChart data={leadSources} layout="vertical" margin={{left:20}}><XAxis type="number" tick={{fill:C.muted,fontSize:9}}/><YAxis type="category" dataKey="s" tick={{fill:C.muted,fontSize:9}} width={100}/><Tooltip contentStyle={ttStyle}/><Legend wrapperStyle={{fontSize:10}}/>
  ...
  return (<div style={{display:"flex",flexDirection:"column",gap:14}}>
    ...
    <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:12}}>
      <Card title="🔗 Conversion Funnel — Page Views">
        <ResponsiveContainer width="100%" height={240}>
          <BarChart data={funnelData}><CartesianGrid strokeDasharray="3 3" stroke={C.border}/><XAxis dataKey="step" tick={{fill:C.muted,fontSize:10}}/><YAxis tick={{fill:C.muted,fontSize:10}}/><Tooltip contentStyle={ttStyle}/>
      ...
      <Card title="📡 Referrer Source — Visits & Conversion Rate">
        <ResponsiveContainer width="100%" height={240}>
          <ComposedChart data={referrers}><CartesianGrid strokeDasharray="3 3" stroke={C.border}/><XAxis dataKey="src" tick={{fill:C.muted,fontSize:9}}/><YAxis tick={{fill:C.muted,fontSize:9}}/><YAxis yAxisId="r" orientation="right" tick={{fill:C.muted,fontSize:9}} domain={[4,7]}/><Tooltip contentStyle={ttStyle}/><Legend wrapperStyle={{fontSize:10}}/>
  ...
  return (<div style={{display:"flex",flexDirection:"column",gap:14}}>
    ...
    <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:12}}>
      <Card title="🔴 Severity Distribution">
        <ResponsiveContainer width="100%" height={220}><PieChart><Pie data={fraudSev} cx="50%" cy="50%" innerRadius={45} outerRadius={80} dataKey="value" label={({name,value})=>`${name}: ${value}`} style={{fontSize:10}}>{fraudSev.map((e,i)=><Cell key={i} fill={e.color}/>)}</Pie><Tooltip contentStyle={ttStyle}/></PieChart></ResponsiveContainer>
      </Card>
      <Card title="📊 Alert Status">
        <ResponsiveContainer width="100%" height={220}><PieChart><Pie data={fraudStatus} cx="50%" cy="50%" innerRadius={45} outerRadius={80} dataKey="value" label={({name,value})=>`${name}: ${value}`} style={{fontSize:9}}>{fraudStatus.map((e,i)=><Cell key={i} fill={e.color}/>)}</Pie><Tooltip contentStyle={ttStyle}/></PieChart></ResponsiveContainer>
      </Card>
      <Card title="🤖 Detection Methods">
        <ResponsiveContainer width="100%" height={220}>
          <BarChart data={detMethods} layout="vertical" margin={{left:10}}><XAxis type="number" tick={{fill:C.muted,fontSize:9}}/><YAxis type="category" dataKey="method" tick={{fill:C.muted,fontSize:9}} width={85}/><Tooltip contentStyle={ttStyle}/>
    ...
    <div style={{display:"grid",gridTemplateColumns:"2fr 1fr",gap:12}}>
      <Card title="🏷️ Fraud Types — Alert Distribution">
        <ResponsiveContainer width="100%" height={220}>
          <BarChart data={fraudTypes}><CartesianGrid strokeDasharray="3 3" stroke={C.border}/><XAxis dataKey="type" tick={{fill:C.muted,fontSize:9}}/><YAxis tick={{fill:C.muted,fontSize:9}}/><Tooltip contentStyle={ttStyle}/>
  ...
  return (<div style={{display:"flex",flexDirection:"column",gap:14}}>
    ...
    <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:12}}>
      <Card title="🔗 MDM Match Tier Distribution">
        <ResponsiveContainer width="100%" height={240}><PieChart><Pie data={mdmMatch} cx="50%" cy="50%" innerRadius={55} outerRadius={90} dataKey="value" label={({name,value})=>`${name}: ${value}`} style={{fontSize:11}}>{mdmMatch.map((e,i)=><Cell key={i} fill={e.color}/>)}</Pie><Tooltip contentStyle={ttStyle}/></PieChart></ResponsiveContainer>
      </Card>
      <Card title="🏷️ Source System Linkage">
        <ResponsiveContainer width="100%" height={240}>
          <BarChart data={sourceLink} layout="vertical" margin={{left:20}}><XAxis type="number" tick={{fill:C.muted,fontSize:9}}/><YAxis type="category" dataKey="name" tick={{fill:C.muted,fontSize:9}} width={110}/><Tooltip contentStyle={ttStyle}/>
